...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '8E979566';
		$this->api_url = 'http://project.cander.in/lcrdsqm/';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["057aa3861c7ef58"])){
				$_SESSION["057aa3861c7ef58"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["057aa3861c7ef58"])){
				$get_data = $this->call_api(
					'POST',
