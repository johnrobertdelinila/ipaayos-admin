...
abstract class REST_Controller extends CI_Controller {
    ...
    private $rate_limit_method = null;
    ...
    public function __construct($config = 'rest')
    {
        ...
        $this->rate_limit_method = (strtolower($this->user_model->get_config_from_db('api_rate_limit_method')) == 'api_key')?'API_KEY':'IP_ADDRESS';
        ...
        //Check LB-Lang and load language file accordingly.
        $_lang = 'HTTP_' . strtoupper(str_replace('-', '_', 'LB-LANG'));
        $has_lang = $this->input->server($_lang);

        if(!empty($has_lang)){
            $language = trim(strip_tags($has_lang));
        }else{
            $language = "english";
        }
        ...
        if ($this->input->is_ajax_request() === FALSE && $this->config->item('rest_ajax_only'))
        {
            ...
            $this->response([
                ...
                $this->config->item('rest_message_field_name') => __('Only AJAX requests are allowed.')
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('force_https') && $this->request->ssl === FALSE)
        {
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => __('Unsupported protocol')
            ], self::HTTP_FORBIDDEN);
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => __('Provided API key is invalid.', $this->rest->key)
            ], self::HTTP_FORBIDDEN);
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === FALSE && $this->_check_access() === FALSE)
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => __('This API key does not have access to the requested resource.')
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE)
        {
            ...
            {
                $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => ($this->rate_limit_method == 'API_KEY')?__('This API key has reached its maximum requests per hour limit, try again later.'):__('This IP Address has reached its maximum requests per hour limit, try again later.')];
            ...
            if($authorized === FALSE)
            {
                ...
                $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => __('This API key does not have enough permissions.')];
                $this->response($response, self::HTTP_UNAUTHORIZED);
        ...
        {
            $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => __('This IP Address has reached the time limit for this method.')];
            $this->response($response, self::HTTP_UNAUTHORIZED);
    ...
    protected function _check_php_session()
    {
        ...
        if ( ! $this->session->userdata($key))
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => __('Unauthorized')
            ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        if (strcasecmp($digest['response'], $valid_response) !== 0)
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => __('Invalid credentials')
            ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _check_blacklist_auth()
    {
        ...
        if (preg_match($pattern, $this->config->item('rest_ip_blacklist')))
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => __('IP denied')
            ], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _check_whitelist_auth()
    {
        ...
        if (in_array($this->input->ip_address(), $whitelist) === FALSE)
        {
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => __('IP unauthorized')
            ], self::HTTP_UNAUTHORIZED);
        }
    }
