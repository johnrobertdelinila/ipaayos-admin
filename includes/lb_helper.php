...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = 'D26005B2';
		$this->api_url = 'https://project.cander.in/lcrdsqm/';
		$this->api_key = '11ADCC0C7D6F933B4399';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["889b917aeabd51d"])){
				$_SESSION["889b917aeabd51d"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["889b917aeabd51d"])){
				$get_data = $this->call_api(
					'POST',
